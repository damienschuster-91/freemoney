...
const CAT_ICON: Record<string, string> = {
  scholarship: "🎓", grant: "💸", trade: "🔧", other: "🎯", local: "📍",
}
        ...
        <div style={{ maxWidth: 960, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 4, background: "rgba(255,255,255,0.08)", padding: 4, borderRadius: 12, width: "fit-content" }}>
            {[
              ...
              { id: "blog" as const, label: "✍️ Blog Tracker" },
            ].map(t => (
            ...
            <div key={item.ein} style={{ ...S.card, borderLeft: `4px solid ${tabCol}` }}>
              <div style={{ padding: "16px 18px", display: "flex", gap: 12, alignItems: "flex-start" }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
                    ...
                    <span style={{ fontSize: 13, color: "#8a9abb", fontWeight: 500 }}>📅 {item.latest_year}</span>
                ...
                <div style={{ display: "flex", gap: 6, flexShrink: 0, flexWrap: "wrap", justifyContent: "flex-end" }}>
                  {tab === "pending" && !isEditing && (<>
                    ...
                    <button onClick={() => reject(item)} className="btn-ghost" style={{ padding: "7px 12px", fontSize: 12, color: "#c0392b", borderColor: "#f5c5c5" }}>✗</button>
                  </>)}
                  {tab === "pending" && isEditing && (<>
                  ...
                  {tab === "approved" && (<>
                    <button onClick={() => startEdit(item)} className="btn-ghost" style={{ padding: "7px 10px", fontSize: 12 }}>✎</button>
              ...
              {isEditing && (
                <div style={{ padding: "0 18px 16px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, borderTop: "1px solid #f0f2f8", paddingTop: 14 }}>
                  ...
                  <div>
                    ...
                    <input value={String(draft.tags ?? "")} onChange={e => setDraft(d => ({ ...d, tags: e.target.value }))} style={S.input} />
                  </div>
                  <div>
              ...
              {isExpanded && !isEditing && (
                <div style={{ padding: "14px 18px", borderTop: "1px solid #f0f2f8" }}>
                  ...
                  {item.notes && <div style={{ marginTop: 10, fontSize: 12, color: "#5a6a8a", background: "#fffbee", padding: "8px 12px", borderRadius: 8 }}>📝 {item.notes}</div>}
                </div>
              )}
            </div>
          )
...
function BlogTracker() {
  ...
  const statusLabel = { draft: "✏️ Draft", written: "📝 Written", published: "✅ Published" }
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 24 }}>
        {([
          { k: "draft" as const, label: "To Write", icon: "✏️" },
          { k: "written" as const, label: "Written", icon: "📝" },
          { k: "published" as const, label: "Published", icon: "✅" },
        ]).map(x => (
          <div key={x.k} onClick={() => setFilter(filter === x.k ? "all" : x.k)} style={{
            ...
            <div style={{ fontSize: 11, color: filter === x.k ? "rgba(255,255,255,0.8)" : "#8a9abb", fontWeight: 600, marginTop: 2 }}>{x.icon} {x.label}</div>
          </div>
        ))}
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ position: "relative", flex: 1, minWidth: 200 }}>
          <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "#8a9abb", fontSize: 14 }}>🔍</span>
              ...
              <div style={{ padding: "14px 18px", display: "flex", gap: 12, alignItems: "flex-start" }}>
                ...
                <div style={{ display: "flex", gap: 5, flexShrink: 0, flexWrap: "wrap", alignItems: "flex-start", justifyContent: "flex-end", maxWidth: 210 }}>
                  {!isEditing ? (<>
                    ...
                    <button onClick={() => { setEditId(post.id); setDraft({ name: post.name, notes: post.notes, blog_url: post.blog_url }) }} className="btn-ghost" style={{ padding: "6px 10px", fontSize: 12 }}>✎</button>
                    <a href={post.url} target="_blank" rel="noopener noreferrer" title="Official page" className="btn-ghost" style={{ padding: "6px 10px", fontSize: 12, textDecoration: "none" }}>🔗</a>
                    <a href={`/blog/${post.id.replace("blog-", "")}`} target="_blank" rel="noopener noreferrer" title="Blog post" className="btn-ghost" style={{ padding: "6px 10px", fontSize: 12, textDecoration: "none" }}>📖</a>
                  </>) : (<>
