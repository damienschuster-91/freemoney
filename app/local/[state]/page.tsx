...
export function generateStaticParams() {
  const states = [...new Set(LOCAL_DATA.map(l => l.state.toLowerCase()))]
...
export default function StatePage({ params }: { params: { state: string } }) {
  ...
  return (
    <main style={{maxWidth:900,margin:"0 auto",padding:"2rem 1rem"}}>
      <Link href="/local" style={{color:"#1a3a6b",fontSize:"0.9rem"}}>← All States</Link>
      <h1 style={{marginTop:"1rem",fontSize:"2rem",fontWeight:700}}>{stateUpper} Local Foundation Scholarships</h1>
      <p style={{color:"#666",marginBottom:"2rem"}}>{listings.length} verified foundation{listings.length>1?"s":""} — all confirmed via IRS 990 filings</p>
        ...
        {listings.map(l => (
          <div key={l.id} style={{border:"1px solid #e2e8f0",borderRadius:12,padding:"1.5rem",background:"#fff"}}>
            ...
            <div style={{display:"flex",gap:"1rem",flexWrap:"wrap",alignItems:"center"}}>
              <span style={{color:"#666",fontSize:"0.85rem"}}>📅 {l.deadline}</span>
              {l.county && <span style={{color:"#666",fontSize:"0.85rem"}}>📍 {l.county}</span>}
              <a href={l.url} target="_blank" rel="noopener noreferrer" style={{color:"#1a3a6b",fontSize:"0.85rem",fontWeight:600}}>Apply →</a>
              {l.propublica_url && <a href={l.propublica_url} target="_blank" rel="noopener noreferrer" style={{color:"#666",fontSize:"0.8rem"}}>📄 990 Filing</a>}
            </div>
          </div>
        ))}
      </div>
